    ...
    if !is_simple {
        match fs::canonicalize(rustc) {
            Ok(canonical) => {
                ...
                if lowered != "rustc" && lowered != "rustc.exe" {
                    return false;
                }
            },
            Err(_) => {
                let file_name = Path::new(rustc)
                    .file_name()
...
fn generate_header() -> Result<(), Box<dyn Error>> {
    ...
    ensure_directory(&generated_include_dir)?;
 ...
 */"#;
    ...
    let new_contents = String::from_utf8(new_file_bytes).map_err(|err| -> Box<dyn Error> {
        format!("header contained invalid UTF-8: {err}").into()
    })?;
